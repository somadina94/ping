...
- **VPS monitor**: Scheduled HTTPS probe of `MONITOR_URL`; alerts via SMTP after consecutive failures, with cooldown and recovery email.
...
PING polls every `MONITOR_INTERVAL_MS` (default 60s). After `MONITOR_FAILURE_THRESHOLD` consecutive failures it sends an alert email, then waits `MONITOR_ALERT_COOLDOWN_MS` before re-alerting while still down. When the target returns `{"status":"ok",...}` after an alert, PING sends a recovery email once.
...
On the instance, create that directory once, install Docker, and place a production `.env` there (rsync never overwrites `.env`). Subsequent pushes sync the repo and run `./deploy.sh`.
